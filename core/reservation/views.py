...
class ReservationView(APIView):
    ...
    def get(self, request):
        ...
        for reservation in reservations:
            if reservation.expired_date > timezone.now() and reservation.reserved_by.count() <= reservation.max_limit_reserve:
                available_configs.append(reservation)
            
        serializer = ReservetionConfigSerializer(available_configs, many=True)
